window.onload = function() {
    let buttons = document.getElementsByTagName('span'), // Select all buttons
    ...
    for (let i = 0; i < buttons.length; i += 1) { // Add onclick events to buttons
    ...
    function invert(i) {
        return function() {
            if (equation.length == 0) {
                ...
                let number = result.innerHTML; // Grab number currently typed in
                popNumberOfDigits(number); // remove number of digits from equation array
                let invert = number * -1; // create inverted number by multiplying by -1
    ...
    function percent(i) {
        return function() {
            let number = result.innerHTML; // Grab number currently typed in
            popNumberOfDigits(number); // remove number of digits from equation array
            let percent = number * 0.01; // create percentage
    ...
    function popNumberOfDigits(number) {
        let arr = number.split(''); // Create an array holding each part of the number (eg. 13 = ['1', '3'] )
